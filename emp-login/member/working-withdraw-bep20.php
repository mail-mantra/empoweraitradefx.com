...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && $_REQUEST['rdm'] != '' && $_REQUEST['action'] != '') {
    ...
    if ($data['rdm'] == '') {
        ...
        $con = $db->connect();
        $q = mysqli_query($con, "select v.*, m.crypto_address_bep20 from pay_working_voucher v join member_details m on v.m_id = m.member_id where v.id='" . $id . "' and v.withdraw_status=0");
        ...
        if (!empty($r)) {
            ...
            if (strpos($message, "error") || $hash == '') {
                $con = $db->connect();
                $tq1 = mysqli_query($con, "insert into api_send_response_bep20(request_no,member_id, amount, response,created_on) values('" . $request_no . "','" . $member_id . "', '" . $amount . "', '" . $response_json . "','" . $now . "')");
                $db->dbDisconnet($con);
                ...
                $tq1 = mysqli_query($con, "insert into api_send_response_bep20(request_no, member_id, to_address, amount, hash, response, created_on) values ('" . $request_no . "','" . $member_id . "','" . $to_address . "', '" . $amount . "', '" . $hash . "','" . $response_json . "','" . $now . "')");
                $tq2 = mysqli_query($con, "update pay_working_voucher set withdraw_status=1, withdraw_date='" . $now . "', updated_by='" . $action_by . "' where id='" . $id . "' and withdraw_status=0");
        ...
        if ($wallet_type == 'working_wallet') {
            $wallet_balance = get_wallet_balance_of_member($con, $member_id, 'working_wallet_balance');
            $transaction_table = 'working_wallet_transaction';
            $balance_table = 'working_wallet_balance';

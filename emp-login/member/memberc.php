...
//include('../third_party/vendor/autoload.php');
$db = new Database();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['submit'] == 'Submit') {
    ...
    if ($data['intro'] == '' || $data['name'] == '' || $data['email'] == '' || $data['mobile'] == '') {
        ...
        // $con = $db->connect();
        // $sql_email = mysqli_query($con, "SELECT member_id from member where email='" . $data['email'] . "'");
        // $db->dbDisconnet($con);
        ...
        $db->dbDisconnet($con);
        if (mysqli_num_rows($sql_pan) >= 1) {
            $_SESSION['e'] = "More than 1 ID is NOT ALLOWED in 1 PAN No.";
            header("Location: $back");
            die;
        }
        ...
        $con = $db->connect();
        $sql_ad = mysqli_query($con, "select member_id from member_details where adhar_no='" . $data['adhar_no'] . "'");
        $db->dbDisconnet($con);
        if (mysqli_num_rows($sql_ad) >= 1) {
            $_SESSION['e'] = "More than 1 ID is NOT ALLOWED in 1 Aadhar ID.";
            header("Location: $back");
            die;
        }
        ...
        $qfr = mysqli_query($con, "select `rate` from `mf_rate` ORDER BY id DESC LIMIT 1");
        $db->dbDisconnet($con);
        if (mysqli_num_rows($q1) == 1) {
            ...
            $mfr = mysqli_fetch_assoc($qfr);
            $usd_rate = $mfr['rate'];

            $topup_amount = $package_amount / $usd_rate;
        } else {
            $topup_amount = 0;
        ...
        if ($wallet_balance < $topup_amount || $wallet_balance <= 0) {
            $_SESSION['e'] = "Your wallet balance is low to register.";
        } else {*/

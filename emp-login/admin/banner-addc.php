...
if($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['submit'] == 'Save') {
    ...
    if($_FILES['banner_pic']['size'] > 0) {
        //getn priveios data
        // banner_pic --- start
        $ad_photo_file = img_save('banner_pic', 1000, 1000, '../assets/images/banners/');
        if($ad_photo_file['msg'] == 'Success') {
            ...
            if($result->num_rows > 0) {
                ...
                $old_image = $row["image"];
                $sql_main = "UPDATE `banners` SET image = '$image_url' WHERE `id` = 1";
            }
            else {
                $sql_main = "INSERT INTO `banners`(`id`, `image`) VALUES ('1', '$image_url')";
            }
            /*
            echo $sql_main;
            die;
    ...
    if($id == '') {
        ...
        $con = $db->connect();
        $sql_del = "DELETE FROM banners WHERE id='" . $id . "'";

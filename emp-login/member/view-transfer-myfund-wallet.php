...
$page_name='view-transfer-myfund-wallet';
    	...
    	<div class="col-lg-12">
        	<div class="row">
                <div class="dashboard-title-2">
                	<div class="caption-2">
                        <h2>View Transfer Fund to Other Member</h2>
                        <p>Transfer Fund to Other Member List</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="table-panel">
            <div class="row">    
				...
				if(isset($_REQUEST['start_date']) && ($_REQUEST['end_date'])){
					$search_with_date_range = " AND (DATE(a.`created_on`) BETWEEN '".$start_date."' AND '".$end_date."') ";
				}
				else{
					$search_with_date_range = "";
				}
				
				$query = "select a.*, b.mem_code, b.name from transfer_myfund_wallet_balance a inner join member b on a.to_member_id = b.member_id where a.from_member_id='".$user_id."' and a.gross_amount>'0.00' ".$search_with_date_range;							
				
				$con=$db->connect();
				$ss = mysqli_query($con,"$query order by a.id desc");
				$db->dbDisconnet($con);
				$t = mysqli_num_rows($ss);
				$tot_page = ceil($t / $uplim);
				$con=$db->connect();
				$sql = mysqli_query($con,"$query order by a.id desc limit $lowlim,$uplim");
				$db->dbDisconnet($con);
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>ID : <?php echo $r1['mem_code']; ?><br>Name : <?php echo $r1['name']; ?></td>	
                                    <td><?php echo CURRENCY_ICON. number_format($gross,2); ?></td>
                                </tr>
                            <?php
                            }
                            ?>

...
if(isset($_POST['api_key']) && $test_api_key == $_POST['api_key']) {
    ...
    try {
        ...
        $income_type = $post_data['income_type'] ?? '';
        if(!$income_type) {
            throw new Exception('Income Type Required');
        }


        // 1. Total records count (no limit)
        $total_stmt = $con->prepare("SELECT COUNT(*) as total FROM `income_fund` WHERE gross_amount>0 AND member_id=? and income_type = ?");
        $total_stmt->bind_param("is", $user_id, $income_type);
        $total_stmt->execute();
        ...
        $stmt->bind_param("isii", $user_id, $income_type, $offset, $limit);
        $stmt->execute();
